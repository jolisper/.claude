...
1. Check if `](../@topics/@{tag}.md)` already appears in NOTE_PATH. If yes: skip.
2. Run `bash -c "test -f '{VAULT}/@topics/@{tag}.md'"`. If non-zero:
   Write `# @{tag}` to `{VAULT}/@topics/@{tag}.md`.
   On failure: report the error and stop.
3. Append `[@{tag}](../@topics/@{tag}.md)` to the tag-link line using Edit:
   - If the note has a tag-link line (a line containing `](../@topics/`): append to it.
