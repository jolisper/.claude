...
Before starting, run `git rev-parse --abbrev-ref --symbolic-full-name @{u}`. If the command fails (no upstream set), get the current branch name with `git rev-parse --abbrev-ref HEAD`. In auto mode (default, no `--no-auto`), proceed directly — push with `git push -u origin <branch>` in Phase 2 to create the upstream, no confirmation needed. If `--no-auto` was passed, propose: "No upstream configured. I'll push with `git push -u origin <branch>` to create it — proceed?" and wait for explicit confirmation; if the user declines, stop.
