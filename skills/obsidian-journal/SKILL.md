...
1. **Existing tags:** From the existing name list, identify those relevant to
   DAY_NOTE, TODAY_NOTES, and LOG_ENTRIES, not already in TAGS.
2. **New tags:** Independently propose up to 3 new tag names based on
   DAY_NOTE, TODAY_NOTES, and LOG_ENTRIES that are not already in TAGS and
   not in the existing list. Use lowercase, hyphen-separated identifiers
   (e.g. `voiceink`, `hardware-setup`, `macos-tools`).
